/**
 * Transform for the geometry model that predates the `new-geometry` migration:
 * `geometry: { epsg, value: { flowGeometry2D | flowGeometry3D | cityGmlGeometry } }`,
 * with `{ x, y, z }` coordinate objects.
 *
 * Runs on any file written by an engine build without `new-geometry`, which
 * includes everything already sitting in the artifact bucket. Left as it was;
 * new work belongs in `transformNextFeature`.
 */
export function transformLegacyFeature(parsedData: any) {
