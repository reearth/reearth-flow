...
/**
 * Initialize the Sentinel service worker, which injects the tile-server token
 * on requests to the protected terravista domain (Google Satellite imagery).
 *
 * Must be called after loadConfig() resolves, before the viewer mounts.
 * No-ops when tileServerToken / tileServerBaseUrl are absent (local dev / OSS),
 * in which case the viewer falls back to ESRI imagery.
 *
 * registerAssetSecurity() handles waiting for the SW to control the page
 * (including hard reload scenarios), so updateToken() is safe to call
 * immediately after.
 */
