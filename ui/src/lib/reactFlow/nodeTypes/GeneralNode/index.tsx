...
const GeneralNode: React.FC<GeneralNodeProps> = ({
  ...
  return (
    <div
      className={`max-w-[200px] min-w-[150px] rounded-md border bg-secondary ${selected ? selectedColor : borderColor} ${data.isDisabled ? "opacity-70" : ""}`}>
      <div className="relative m-1 flex h-[25px] items-center gap-1 rounded-sm">
        <div
          className={`flex justify-center self-center rounded-sm border p-1 align-middle ${selected ? selectedColor : borderColor} ${selected ? selectedBackgroundColor : backgroundColor}`}>
          {type === "reader" ? (
