...
const CanvasSearch: React.FC = () => {
  ...
  const filteredReaders = nodes.filter(
    (node) =>
      node.type === "reader" && valueInSearch(searchValue, node.data.name),
  );
  const filteredWriters = nodes.filter(
    (node) =>
      node.type === "writer" && valueInSearch(searchValue, node.data.name),
  );
  const filteredTransformers = nodes.filter(
    (node) =>
      node.type === "transformer" && valueInSearch(searchValue, node.data.name),
  );
