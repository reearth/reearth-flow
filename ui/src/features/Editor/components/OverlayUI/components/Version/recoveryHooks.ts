...
// Recovery mode wants "get me back to any working state" for a project
// that will not open at all (see @flow/errors ProjectCorruptionError and
// its consumer, the workspace route's error boundary). The raw update log
// (`projectHistory`) is the right source for that: it is more granular
// than named snapshots, and `previewSnapshot`/`rollbackProject` speak the
// same update-log `version` space as `projectHistory` entries. This is a
// restoration of the pre-snapshot-history behaviour, unchanged, and must
// stay on the update log rather than NamedSnapshot data — see
// ../Version/hooks.ts for why those two ID spaces are not interchangeable.
export default ({
  projectId,
  yDoc,
  onDialogClose,
