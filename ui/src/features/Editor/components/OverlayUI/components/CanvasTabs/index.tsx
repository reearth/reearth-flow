    ...
    <div className="absolute left-0 bottom-0 flex border-t border-r border-zinc-700 rounded-tr">
      ...
      <div className="flex max-w-[50vw] overflow-auto">
        {editingCustomTransformers.map((transformer, idx) => (
          <div
            key={transformer.id}
            className={`relative w-28 px-2 py-0.5 bg-zinc-900 border-r border-zinc-700 text-zinc-500 transition-colors hover:bg-zinc-800 hover:text-zinc-400 cursor-pointer group ${editingCustomTransformers.length === idx + 1 ? "rounded-tr" : undefined}`}>
