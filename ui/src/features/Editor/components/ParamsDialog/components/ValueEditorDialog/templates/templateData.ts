...
export const getExpressionTemplates = (t: (key: string) => string): ExpressionTemplate[] => [
  // Advanced File Operations (beyond Simple Builder capabilities)
  {
    ...
    placeholders: [
      { key: 'filePath', description: t('The file path expression'), defaultValue: 'env.get("__value").cityGmlPath' }
    ],
...
attributes["{{attributeName}}"] ?? {{defaultValue}}`,
    placeholders: [
      { key: 'attributeName', description: t('CityGML attribute name'), defaultValue: 'bldg:boundedBy' },
      { key: 'defaultValue', description: t('Default value if attribute missing'), defaultValue: '[]' }
    ],
...
attributes["bldg:boundedBy"] ?? []`,
  ...
  },
  {
    ...
    placeholders: [
      { key: 'filePath', description: t('File path to extract from'), defaultValue: 'env.get("__value").cityGmlPath' }
    ],
...
if building_id_attribute.len == 1 {
...

  {
    ...
    placeholders: [
      { key: 'geometryAttribute', description: t('Geometry type attribute name'), defaultValue: 'geomTag' },
      { key: 'allowedTypes', description: t('Comma-separated allowed types'), defaultValue: '"gml:MultiSurface", "gml:Solid"' }
    ],
...
if gml_name == "{{primaryType}}" {
  ...
  "{{defaultLabel}}"
    ...
    placeholders: [
      { key: 'primaryType', description: t('Primary feature type to check'), defaultValue: 'Building' },
      { key: 'primaryLabel', description: t('Label for primary type'), defaultValue: 'building' },
      { key: 'secondaryType', description: t('Secondary feature type'), defaultValue: 'BuildingPart' },
      { key: 'secondaryLabel', description: t('Label for secondary type'), defaultValue: 'building_part' },
      { key: 'defaultLabel', description: t('Default label'), defaultValue: 'other' }
    ],
  ...
  // Array Processing - Collection operations
  {
    ...
    placeholders: [
      { key: 'arrayAttribute', description: t('Array attribute name'), defaultValue: 'relatedXMLTags' }
    ],
    preview: 'collection::join_array((env.get("__value").relatedXMLTags ?? []), ",")',
    usageExample: t('Convert array of XML tags into comma-separated string for output')
  },
  {
    id: 'count-by-type',
...
{{arrayName}}.reduce(|sum| { if this["type"] == "{{targetType}}" { sum + 1 } else { sum } }, 0)`,
    placeholders: [
      { key: 'arrayName', description: t('Variable name for array'), defaultValue: 'bounded_by' },
      { key: 'arrayExpression', description: t('Expression that returns array'), defaultValue: 'attributes["bldg:boundedBy"]' },
      { key: 'targetType', description: t('Type to count'), defaultValue: 'bldg:RoofSurface' }
    ],
...
bounded_by.reduce(|sum| { if this["type"] == "bldg:RoofSurface" { sum + 1 } else { sum } }, 0)`,
  ...
  },
  {
    ...
    placeholders: [
      { key: 'arrayAttribute', description: t('Array attribute name'), defaultValue: 'package' }
    ],
...
{{attributeName}}.len() > 0`,
    placeholders: [
      { key: 'attributeName', description: t('Variable name for attribute'), defaultValue: 'building_id' },
      { key: 'attributeKey', description: t('CityGML attribute key'), defaultValue: 'uro:buildingIDAttribute' }
    ],
...
value in [{{allowedValues}}]`,
    placeholders: [
      { key: 'attributeName', description: t('Attribute to validate'), defaultValue: 'featureType' },
      { key: 'allowedValues', description: t('Comma-separated allowed values'), defaultValue: '"Building", "BuildingPart", "BuildingInstallation"' }
    ],
    preview: `let value = env.get("__value").featureType ?? "";
value in ["Building", "BuildingPart", "BuildingInstallation"]`,
    usageExample: t('Validate that feature type is one of allowed building types')
  }
];
...
export function getTemplatesByCategory(category: keyof ReturnType<typeof getTemplateCategories>, t: (key: string) => string) {
  return getExpressionTemplates(t).filter(template => template.category === category);
}

// Helper function to search templates
export function searchTemplates(query: string, t: (key: string) => string) {
  const lowerQuery = query.toLowerCase();
  return getExpressionTemplates(t).filter(template =>
