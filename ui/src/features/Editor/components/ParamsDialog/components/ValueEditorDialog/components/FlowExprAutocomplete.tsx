...
export type FlowExprAutocompleteRef = {
  /** Called by the parent textarea keydown so navigation keys reach the
   *  autocomplete even though propagation is stopped before document. */
...
const FlowExprAutocomplete = forwardRef<FlowExprAutocompleteRef, Props>(
  (
    { textareaRef, value, onSuggestionSelect, visible, onVisibilityChange },
    ref,
  ) => {
    const t = useT();
    ...
    // Keep a ref to the current suggestions/selectedIndex so the imperative
    // handler always sees fresh values without stale closure issues.
    ...
    // Expose navigation handler to parent so it can be invoked directly from
    // the textarea's onKeyDown (the document listener never fires because the
    // parent calls stopPropagation to block Radix Dialog's ESC handler).
    useImperativeHandle(
      ref,
      () => ({
        handleNavigationKey: (key: string) => {
          ...
          switch (key) {
            ...
            case "ArrowUp":
              setSelectedIndex(
                (prev) =>
                  (prev - 1 + currentSuggestions.length) %
            ...
            case "Tab":
              if (currentSuggestions[currentIndex]) {
                onSuggestionSelect(currentSuggestions[currentIndex]);
              }
              break;
            case "Escape":
              onVisibilityChange(false);
      ...
      [visible, onSuggestionSelect, onVisibilityChange],
    );
