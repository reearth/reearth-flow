 ...
 */
export function replacePlaceholders(code: string, values: PlaceholderValue[]): string {
  let result = code;
...
export function processTemplate(
  ...
  const populatedCode = replacePlaceholders(template.rhaiCode, 
...
export function validatePlaceholderValue(
  ...
  if (key.toLowerCase().includes('attribute') && !value.match(/^[a-zA-Z][a-zA-Z0-9:_-]*$/)) {
    return { isValid: false, error: 'Invalid attribute name format' };
  }
  
  if (key.toLowerCase().includes('filename') && value.includes('/')) {
    return { isValid: false, error: 'Filename should not contain path separators' };
  }
...
export function generateTemplatePreview(
  ...
  if (!processed.hasPlaceholders || processed.placeholders.every(p => p.currentValue)) {
