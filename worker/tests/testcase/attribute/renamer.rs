...
async fn test_add_prefix() {
    ...
    let default = executor
        .start().await.unwrap().get(&Port::new("default")).unwrap().clone().unwrap();
    match default {
        ActionValue::Array(xs) => {
            match &xs[0] {
                ActionValue::Map(kv) => 
                    match kv.get("foo_foo").unwrap() {
                        ActionValue::Map(kv) => {
                            kv.get("foo_bar1");
                            kv.get("foo_bar2");
                        },
...
async fn test_regular_expression_replace() {
    let executor = init_test_runner("attribute/renamer/regular_expression_replace", vec!["renamer.json"]).await;
    let default = executor
        .start().await.unwrap().get(&Port::new("default")).unwrap().clone().unwrap();
    match default {
        ActionValue::Array(xs) => {
