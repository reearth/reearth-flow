...
fn handle_city_gml_geometry(
    ...
    let (_, receiver_sorted) = mpsc::sync_channel(2000); // TODO
    let ellipsoid = nusamai_projection::ellipsoid::wgs84();
    let tile_id_conv = TileIdMethod::Hilbert;

    let contents: Arc<Mutex<Vec<TileContent>>> = Default::default();

    let receiver_sorted: mpsc::Receiver<(u64, String, Vec<Vec<u8>>)> = receiver_sorted;

    receiver_sorted.into_iter().par_bridge().try_for_each(
        |(tile_id, typename, _)| -> Result<(), SinkError> {
            // Tile information
            ...
            let storage = storage_resolver
                .resolve(output)
                .map_err(crate::errors::SinkError::file_writer)?;
            let uri_path = format!("{:?}/{}", output.path(), content_path);
