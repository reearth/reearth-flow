...
impl FlowProjectRedisDataManager {
    ...
    async fn set_state_data(&self, state_update: Vec<u8>, state_updated_by: Vec<String>, skip_lock: bool) -> Result<(), Box<dyn Error>> {
        ...
        if skip_lock {
            ...
            connection_guard.set(self.state_key(), &encoded_state_update).await?;
            connection_guard.set(self.state_updated_by_key(), &updated_by_json).await?;
        } else {
            self.global_lock.lock_state(&self.project_id, 5000, |_lock_guard| {
                Box::pin(async {
                    let connection = self.redis_client.connection();
                    let mut connection_guard = connection.lock().await;
                    connection_guard.set(self.state_key(), &encoded_state_update).await?;
    ...
    pub async fn push_update(&self, update: Vec<u8>, updated_by: String) -> Result<(), Box<dyn Error>> {
        ...
        connection_guard.xadd(&self.state_updates_key(), "*", fields).await?;
    
        let timestamp = chrono::Utc::now().timestamp().to_string();
        connection_guard.set(self.last_updated_at_key(), &timestamp).await?;
        ...
        if let Some(active_session_id) = self.active_editing_session_id().await? {
            ...
            if let Some(current_session_id) = editing_session.session_id.as_ref() {
                if active_session_id == *current_session_id {
                    connection_guard.del(&[self.active_editing_session_id_key()]).await?;
                }
            }
        }
    ...
    async fn execute_merge_updates(&self) -> Result<(Vec<u8>, Vec<String>), Box<dyn Error>> {
        ...
        let (merged_update, last_update_id, updates_by) = if let Some((merged_update, last_update_id, updates_by)) = merged_stream_update {
        ...
        if let Some(last_update_id) = last_update_id {
            ...
            self.redis_client.xdel(&self.state_updates_key(), &[last_update_id.as_str()]).await?;
        }
    ...
    async fn lock_and_execute_merge_updates(&self) -> Result<(Vec<u8>, Vec<String>), Box<dyn Error>> {
        self.global_lock.lock_updates(&self.project_id, 5000, |_| {
            Box::pin(async move {
                Ok::<(), Box<dyn Error>>(())
            })
        })
