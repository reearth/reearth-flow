...
use flow_websocket_domain::repository::{ProjectEditingSessionRepository, ProjectSnapshotRepository};
use std::sync::Arc;
use std::error::Error;
...
impl ManageEditSessionService {
    ...
    pub async fn process(&self, mut data: ManageProjectEditSessionTaskData) -> Result<(), Box<dyn Error>> {
        let mut session = self.session_repository.get_active_session(&data.project_id).await?
        ...
        self.create_snapshot_if_required(&mut session, &mut data).await?;
        self.end_editing_session_if_conditions_met(&mut session, &data).await?;
        self.complete_job_if_met_requirements(&session, &data).await?;

        Ok(())
    }

    async fn update_client_count(&self, session: &mut ProjectEditingSession, data: &mut ManageProjectEditSessionTaskData) -> Result<(), Box<dyn Error>> {
        ...
        if current_client_count == 0 && old_client_count != current_client_count && data.clients_disconnected_at.is_none() {
    ...
    async fn merge_updates(&self, session: &mut ProjectEditingSession, data: &mut ManageProjectEditSessionTaskData) -> Result<(), Box<dyn Error>> {
        session.merge_updates().await?;
        data.last_merged_at = Some(Utc::now());
        Ok(())
    }

    async fn create_snapshot_if_required(&self, session: &mut ProjectEditingSession, data: &mut ManageProjectEditSessionTaskData) -> Result<(), Box<dyn Error>> {
        if let Some(last_snapshot_at) = data.last_snapshot_at {
            ...
            if snapshot_time_delta > MAX_SNAPSHOT_DELTA {
                ...
                self.snapshot_repository.create_snapshot(flow_websocket_domain::snapshot::ProjectSnapshot {
                    metadata: flow_websocket_domain::snapshot::ProjectMetadata {
                    ...
                    delete: flow_websocket_domain::snapshot::ObjectDelete {
                        deleted: false,
                        delete_after: None,
                    },
    ...
    async fn end_editing_session_if_conditions_met(&self, session: &mut ProjectEditingSession, data: &ManageProjectEditSessionTaskData) -> Result<(), Box<dyn Error>> {
        let client_count = session.get_client_count().await?;
        
        if let Some(clients_disconnected_at) = data.clients_disconnected_at {
            let current_time = Utc::now();
            let clients_disconnection_elapsed_time = (current_time - clients_disconnected_at).num_milliseconds();
    ...
    async fn complete_job_if_met_requirements(&self, session: &ProjectEditingSession, data: &ManageProjectEditSessionTaskData) -> Result<(), Box<dyn Error>> {
