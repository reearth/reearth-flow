            ...
            match op {
                ExecutorOperation::Op { ctx } => {
                    ...
                    if result.is_err() {
                        ...
                        has_failed = true;
                        tracing::warn!("Sink node {} processing failed for feature {:?}", 
                            self.node_handle.id, ctx.feature.id);
                    }
                ...
                ExecutorOperation::Terminate { ctx } => {
                    ...
                    if is_terminated.iter().all(|value| *value) {
                        ...
                        let final_status = if has_failed {
                            tracing::warn!("Sink node {} final status is Failed", self.node_handle.id);
                            NodeStatus::Failed
                        } else {
                            tracing::info!("Sink node {} final status is Completed", self.node_handle.id);
