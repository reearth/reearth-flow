...
use reearth_flow_types::{lod::LodMask, Attribute, AttributeValue, CitygmlFeatureExt, Feature};
...
impl Processor for FeatureLodFilter {
    fn process(
        ...
        let Some(lod) = feature.lod_mask() else {
...
impl FeatureLodFilter {
    ...
    fn feature_with_single_lod(feature: &Feature, max_lod: u8) -> Feature {
        ...
        let actual_lod = if let Some(lod_mask) = feature.lod_mask().as_ref() {
        ...
        if let Some(target_lod) = actual_lod {
            ...
            if filtered_feature.lod_mask().is_some() {
                let mut new_lod_mask = LodMask::default();
                new_lod_mask.add_lod(target_lod);
                filtered_feature.update_lod_mask(new_lod_mask);
            }
        }
