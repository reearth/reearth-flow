...
fn discover_tests(testdata_dir: &Path) -> Result<Vec<TestCase>> {
    ...
    if !testdata_dir.exists() {
        println!("cargo:warning=Test data directory does not exist: {:?}", testdata_dir);
...
fn generate_test_code(test_cases: &[TestCase], testdata_dir: &Path) -> Result<TokenStream> {
    ...
    for test_case in test_cases {
        ...
        let fixture_dir_str = testdata_dir
            .parent()
            .unwrap()
            .to_string_lossy()
            .to_string();
        ...
        let skip_attr = if test_case.profile.skip {
            let reason = test_case.profile.skip_reason.as_deref().unwrap_or("Skipped");
            quote! { #[ignore = #reason] }
        } else {
            quote! {}
        };
