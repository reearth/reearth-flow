...
impl TestContext {
    pub fn new(test_name: String, test_dir: PathBuf, fixture_dir: PathBuf, profile: WorkflowTestProfile) -> Result<Self> {
    ...
    pub fn load_workflow(&self) -> Result<Workflow> {
        let workflow_path = self.fixture_dir.join("workflow").join(&self.profile.workflow_path);
    ...
    pub fn run_workflow(&self, mut workflow: Workflow) -> Result<()> {
        ...
        test_variables.insert("outputPath".to_string(), self.temp_dir.display().to_string());
        test_variables.insert("currentPath".to_string(), self.temp_dir.display().to_string());
    ...
    pub fn verify_intermediate_data(&self) -> Result<()> {
        for assertion in &self.profile.intermediate_assertions {
            ...
            if !edge_data_path.exists() {
                ...
                anyhow::bail!("Intermediate data not found for edge {}: {:?}", assertion.edge_id, edge_data_path);
            }
            ...
            self.compare_data(&actual_data, &expected_data, &assertion.comparison, assertion.except.as_ref())?;
        }
        Ok(())
    }
    ...
    fn verify_json_equals(&self, output: &TestOutput) -> Result<()> {
        if let Some(expected_file_name) = &output.expected_file {
            ...
            let expected: serde_json::Value = serde_json::from_str(&fs::read_to_string(&expected_file)?)?;
            let actual: serde_json::Value = serde_json::from_str(&fs::read_to_string(&actual_file)?)?;
            
            assert_eq!(actual, expected, "JSON output mismatch for {}", self.test_name);
        }
        Ok(())
    }
        ...
        if let Some(expected_file_name) = &output.expected_file {
            ...
            if let Some(expected_inline) = &output.expected_inline {
                if let Some(expected_count) = expected_inline.as_u64() {
                    assert_eq!(count as u64, expected_count, "File count mismatch for {}", self.test_name);
                }
            }
        }
        Ok(())
    ...
    fn compare_data(&self, actual: &str, expected: &str, method: &ComparisonMethod, except: Option<&ExceptFields>) -> Result<()> {
        match method {
    ...
    fn compare_tsv(&self, actual: &str, expected: &str, except: Option<&ExceptColumns>) -> Result<()> {
        ...
        println!("TSV comparison debug: actual columns = {:?}", actual_columns);
        println!("TSV comparison debug: expected columns = {:?}", expected_columns);
        
        // Filter out excluded columns
        let actual_cols_filtered: Vec<&str> = actual_columns.iter()
            .filter(|col| {
        ...
        let expected_cols_filtered: Vec<&str> = expected_columns.iter()
            .filter(|col| {
                if let Some(except) = except {
                    !except.contains(col)
                } else {
        ...
        if actual_cols_sorted != expected_cols_sorted {
            anyhow::bail!(
                ...
                expected_cols_sorted, actual_cols_sorted
            );
        }
        ...
        for (row_idx, (actual_row, expected_row)) in actual_lines.iter().skip(1).zip(expected_lines.iter().skip(1)).enumerate() {
            let actual_values: Vec<&str> = actual_row.split('\t').collect();
            let expected_values: Vec<&str> = expected_row.split('\t').collect();
            
            if actual_values.len() != actual_columns.len() || expected_values.len() != expected_columns.len() {
            ...
            for (expected_col_idx, expected_col_name) in expected_columns.iter().enumerate() {
                ...
                if let Some(actual_col_idx) = actual_columns.iter().position(|col| col == expected_col_name) {
    ...
    fn apply_filter_to_json(&self, json: &serde_json::Value, filter: &str) -> Result<serde_json::Value> {
    ...
    fn construct_object(&self, json: &serde_json::Value, filter: &str) -> Result<serde_json::Value> {
...
mod tests {
    ...
    fn test_json_filter_functionality() -> Result<()> {
        ...
        let ctx = TestContext::new("test_json_filter".to_string(), test_dir, fixture_dir, profile)?;
