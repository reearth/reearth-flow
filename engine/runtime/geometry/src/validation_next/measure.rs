...
use super::{open_ring, signed_area_2d, ValidationReport};
...
/// plane: the minimum height of their 3D convex hull, relative to the hull's
/// diameter, exceeds `tolerance` (a dimensionless ratio). A point set with no
/// 3D hull is flat and passes. Rings must be stored closed with finite
/// coordinates. The position is the exterior ring as a LineString.
pub(crate) fn check_planarity_3d<'a>(
    ...
    let hull_points: Vec<Coordinate3D<f64>> = points
        ...
        .collect();
    // No non-degenerate tetrahedron within the guard: effectively flat.
    let Some(hull) = quick_hull_3d(&hull_points, tolerance * diagonal * 0.01) else {
        return;
    };
    ...
    let max_height = hull_diameter(vertices);
    if max_height == 0.0 {
        return;
    }
    if hull_min_height(vertices, triangles) / max_height > tolerance {
        report.push(Geometry::Euclidean3D(Euclidean3DGeometry::LineString(
...
/// edges, so both families are tested.
fn hull_min_height(vertices: &[Coordinate3D<f64>], triangles: &[[usize; 3]]) -> f64 {
