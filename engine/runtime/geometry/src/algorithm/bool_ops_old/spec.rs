...
use crate::{
    algorithm::{sweep::LineOrPoint, GeoFloat},
    types::{
        multi_line_string::MultiLineString2D, multi_polygon::MultiPolygon2D, no_value::NoValue,
    },
};

use super::{LineAssembly, OpType, RegionAssembly};
...
pub trait Spec<T: GeoFloat> {
    ...
    /// `geom` belongs to.
    fn output(&mut self, regions: [Self::Region; 2], geom: LineOrPoint<T, NoValue>, idx: usize);
...
impl<T: GeoFloat> Spec<T> for BoolOp<T> {
    ...
    fn output(&mut self, regions: [Self::Region; 2], geom: LineOrPoint<T, NoValue>, _idx: usize) {
...
impl<T: GeoFloat> Spec<T> for ClipOp<T> {
    type Region = Region;
    type Output = MultiLineString2D<T>;
    ...
    fn finish(self) -> Self::Output {
        MultiLineString2D::new(self.assembly.finish())
    }
}
