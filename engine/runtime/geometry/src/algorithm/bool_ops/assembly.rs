...
impl<T: GeoFloat> RegionAssembly<T> {
    ...
    pub fn finish(self) -> MultiPolygon2D<T> {
        ...
        for (ring_idx, ring) in rings.iter().enumerate() {
            ...
            polygons.push(Polygon2D::new(exterior, holes));
        }
...
pub struct LineAssembly<T: GeoFloat> {
    segments: Vec<VecDeque<SweepPoint<T, NoValue>>>,
    end_points: BTreeMap<(usize, SweepPoint<T, NoValue>), (usize, bool)>,
}

impl<T: GeoFloat> LineAssembly<T> {
    /// Adds  `geom` to the assembly, specifying the index of the geometry
    /// (i.e., the shape).
    pub fn add_edge(&mut self, geom: LineOrPoint<T, NoValue>, geom_idx: usize) {
    ...
    pub fn finish(self) -> Vec<LineString2D<T>> {
        self.segments
            .into_iter()
            .map(|pts| LineString2D::from_iter(pts.into_iter().map(|pt| *pt)))
            .collect()
    }
}
...
fn split_ring<T: GeoFloat, F: FnMut(LineString2D<T>)>(
    ls: &LineString2D<T>,
    mut cb: F,
) -> LineString2D<T> {
    ...
    for coord in ls.0.iter().copied() {
        if let Some(idx) = pts_map.get(&SweepPoint::from(coord)) {
            let new_ls: LineString2D<_> = exterior
                .drain(idx..)
                .inspect(|pt| {
...
struct Snake<T: GeoFloat> {
    ...
    points: Vec<SweepPoint<T, NoValue>>,
...
impl<T: GeoFloat> Snake<T> {
    ...
    pub fn new(
        start: SweepPoint<T, NoValue>,
        end: SweepPoint<T, NoValue>,
        pair: usize,
        region: WindingOrder,
        parent_snake_idx: usize,
    ) -> Self {
    ...
    pub fn into_ring<F: FnMut(usize)>(
        ...
        let ls = LineString2D::new(output);
        Some(Ring {
            ls,
            is_hole,
            parent_snake_idx,
...
impl<T: GeoFloat> From<LineOrPoint<T, NoValue>> for Segment<T> {
    fn from(geom: LineOrPoint<T, NoValue>) -> Self {
        Segment {
            geom,
