...
pub(crate) struct EdgeEndBundleStar<T, Z>
where
    T: GeoFloat,
    Z: GeoFloat,
{
    edge_map: std::collections::BTreeMap<EdgeEndKey<T, Z>, EdgeEndBundle<T, Z>>,
    point_in_area_location: Option<[CoordPos; 2]>,
}

#[derive(Clone, Debug)]
pub(crate) struct LabeledEdgeEndBundleStar<T, Z>
where
    T: GeoFloat,
    Z: GeoFloat,
{
    edges: Vec<LabeledEdgeEndBundle<T, Z>>,
}

impl<T: GeoFloat, Z: GeoFloat> LabeledEdgeEndBundleStar<T, Z> {
    pub(crate) fn new(
        edges: Vec<LabeledEdgeEndBundle<T, Z>>,
        graph_a: &GeometryGraph<T, Z>,
        graph_b: &GeometryGraph<T, Z>,
    ) -> Self {
    ...
    fn compute_labeling(&mut self, graph_a: &GeometryGraph<T, Z>, graph_b: &GeometryGraph<T, Z>) {
    ...
    fn edge_end_bundles_iter(&self) -> impl Iterator<Item = &LabeledEdgeEndBundle<T, Z>> {
        self.edges.iter()
    }

    fn edge_end_bundles_iter_mut(
        &mut self,
    ) -> impl Iterator<Item = &mut LabeledEdgeEndBundle<T, Z>> {
        self.edges.iter_mut()
    }
...
impl<T, Z> EdgeEndBundleStar<T, Z>
where
    T: GeoFloat,
    Z: GeoFloat,
{
    ...
    fn edge_end_bundles_iter(&self) -> impl Iterator<Item = &EdgeEndBundle<T, Z>> {
        self.edge_map.values()
    }

    fn edge_end_bundles_iter_mut(&mut self) -> impl Iterator<Item = &mut EdgeEndBundle<T, Z>> {
