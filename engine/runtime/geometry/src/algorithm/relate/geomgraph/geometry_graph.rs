...
use crate::{
    ...
    types::{
        coordinate::Coordinate, line::Line, line_string::LineString, point::Point, polygon::Polygon,
    },
};
...
use super::{
    ...
    line_intersector::LineIntersector,
...
{
    arg_index: usize,
    parent_geometry: &'a GeometryCow<'a, T, Z>,
    use_boundary_determination_rule: bool,
    planar_graph: PlanarGraph<T, Z>,
}
    ...
    Z: GeoFloat,
{
    ...
    pub fn is_boundary_node(&self, coord: Coordinate<T, Z>) -> bool {
    ...
    pub fn nodes_iter(&self) -> impl Iterator<Item = &CoordNode<T, Z>> {
        self.planar_graph.nodes.iter()
    }
}

impl<'a, T, Z> GeometryGraph<'a, T, Z>
where
    T: GeoFloat,
    Z: GeoFloat,
{
    pub fn new(arg_index: usize, parent_geometry: &'a GeometryCow<T, Z>) -> Self {
    ...
    fn create_edge_set_intersector() -> Box<dyn EdgeSetIntersector<T, Z>> {
    ...
    fn boundary_nodes(&self) -> impl Iterator<Item = &CoordNode<T, Z>> {
        self.planar_graph.boundary_nodes(self.arg_index)
    }

    pub fn add_geometry(&mut self, geometry: &GeometryCow<T, Z>) {
    ...
    fn add_polygon_ring(
        ...
        let mut coords: Vec<Coordinate<T, Z>> = Vec::with_capacity(linear_ring.0.len());
    ...
    fn add_polygon(&mut self, polygon: &Polygon<T, Z>) {
    ...
    fn add_line_string(&mut self, line_string: &LineString<T, Z>) {
        if line_string.is_empty() {
            return;
        }

        let mut coords: Vec<Coordinate<T, Z>> = Vec::with_capacity(line_string.0.len());
    ...
    fn add_line(&mut self, line: &Line<T, Z>) {
    ...
    fn add_point(&mut self, point: &Point<T, Z>) {
    ...
    fn insert_point(&mut self, arg_index: usize, coord: Coordinate<T, Z>, position: CoordPos) {
        let node: &mut CoordNode<T, Z> = self.add_node_with_coordinate(coord);
        node.label_mut().set_on_position(arg_index, position);
    }

    /// Add the boundary points of 1-dim (line) geometries.
    fn insert_boundary_point(&mut self, coord: Coordinate<T, Z>) {
    ...
    fn add_self_intersection_node(&mut self, coord: Coordinate<T, Z>, position: CoordPos) {
