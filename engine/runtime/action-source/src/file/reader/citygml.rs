...
pub struct CityGmlReaderParam {
    /// # Flatten Attributes
    /// When enabled, flattens nested CityGML attributes into top-level attributes. Defaults to false.
    ...
    for entity in entities {
        ...
        for mut ent in entities {
            ...
            if let Some(id) = &citygml_gml_id {
                feature.update_feature_id(id.clone());
            }
            if let Some(ft) = &citygml_feature_type {
                feature.update_feature_type(ft.clone());
            }
