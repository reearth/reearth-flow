...
pub fn try_from(s: &str) -> crate::Result<DateTime<Utc>> {
    if let Ok(v) = DateTime::parse_from_rfc3339(s).map_err(crate::Error::datetime) {
        Ok(v.into())
    } else if let Ok(v) = DateTime::parse_from_str(s, "%Y/%m/%d %H:%M:%S") {
        Ok(v.into())
    } else if let Ok(v) = DateTime::parse_from_str(s, "%Y-%m-%d %H:%M:%S") {
        Ok(v.into())
