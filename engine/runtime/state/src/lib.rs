...
impl State {
    ...
    // /// Reads all lines of intermediate data stored as jsonl/jsonl.zst and deserializes them.
    // pub fn read_jsonl_sync<T>(&self, id: &str) -> Result<Vec<T>>
    // where
    //     T: DeserializeOwned,
    // {
    //     let p = self.id_to_location(id, self.jsonl_ext());

    //     // Assumes that append_sync or similar has been called beforehand
    //     let bytes = self.storage.get_sync(p.as_path()).map_err(Error::other)?;
