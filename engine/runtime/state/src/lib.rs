...
impl State {
    ...
    /// Copies the JSONL or JSONL.zst file identified by `id` from `src` into `self` (blocking, CLI-friendly).
    ...
    /// Copies the JSONL or JSONL.zst file identified by `id` from `src` into `self` (non-blocking, worker-friendly).
