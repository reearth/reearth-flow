...
impl State {
    ...
    /// Lists all JSONL file stems in this State's root directory (blocking).
    /// Returns stems like "abc-uuid", "parent-uuid.node-uuid.default", etc.
    pub fn list_jsonl_ids_sync(&self) -> Vec<String> {
        let mut out = Vec::new();
    ...
    /// Lists all JSONL file stems in this State's root directory (non-blocking).
    /// Returns stems like "abc-uuid", "parent-uuid.node-uuid.default", etc.
    pub async fn list_jsonl_ids_async(&self) -> Vec<String> {
        let mut out = Vec::new();
