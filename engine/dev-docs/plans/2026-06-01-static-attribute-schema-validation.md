...
> **STATUS — e2e reached, errors now fire (2026-06-01).** `reearth-flow build`/`check` runs end-to-end on the real PLATEAU workflows in-repo (`worker/workflow/cms/plateau4/quality-check/bldg/template/workflow.yml` and `.../data-convert/bldg/...`). It expands the 4 `!include`s, builds the full subgraph-expanded DAG (~87 nodes, routers/mergers, 20+ action types), runs the inference pass without panicking, exit 0. `--show-schema` dumps the inferred per-node schemas: on the quality-check workflow the pass records **87 nodes, 395 inferred attribute fields, 49 `(open)` ports, 20 `Maybe`-presence fields**.
>
> **The validator now catches real reference errors end-to-end.** `AttributeMapper` has *replace* semantics, so its output schema is **closed** (`open=false`) — a downstream node referencing an attribute the mapper does not produce is flagged. All three severities are proven by real-workflow fixtures (`runtime/tests/fixture/workflow/schema_infer/reference_{error,ok,warning}.yml`) + cli integration tests:
> - missing attribute against a closed schema → **ERROR**, `build` exits non-zero;
> - satisfied reference → **passes** (no false positive);
> - `Presence::Maybe` reference (parent/child mapper) → **WARNING**, build still exits 0.
>
> **Typed inference also proven (widen).** `StatisticsCalculator` (used 4× in the real PLATEAU quality-check workflow) emits a fresh CLOSED *typed* schema on its `default` port — `group_by` attrs → **String**, calculations → **Number**, input attrs dropped — and identity passthrough on its `complete` port. The `typed_reference_error.yml` fixture + `build_typed_reference_error_with_known_types` test prove a missing reference is caught end-to-end AND that the surviving fields carry concrete inferred types (`region: String`, `total: Number`), not just `Unknown`. So both name-tracking (via `AttributeMapper`) and typed inference (via `StatisticsCalculator`/`DateTimeConverter`) reach real validation errors end-to-end.
>
> Implemented transfer functions: `AttributeManager`, `AttributeMapper`, `StatisticsCalculator`, `DateTimeConverter`, `FeatureFilter` (others fall back to schema-transparent passthrough; sources seed `open`). Tests: 275 (runtime+processor) + 7 (cli: 2 PLATEAU e2e + 4 reference-validation e2e + 1 valid) + others, all green; fmt+clippy clean. Note the PLATEAU workflows themselves still report 0 errors because their *own* sources are expression-driven (`open`) and their reference targets are satisfied; the error path is reachable wherever an `AttributeMapper`/`StatisticsCalculator` (or any future closed-schema producer) sits upstream of a reference. Remaining to widen further: seed closed schemas from typed *sources* (CSV header columns, DB catalog — note: current source readers derive columns from file content at runtime, so this needs a new declared-schema param, deliberately not added here) and add more transfer functions.
