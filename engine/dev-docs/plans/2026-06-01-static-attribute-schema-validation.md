...
>
> Implemented transfer functions: `AttributeManager`, `AttributeMapper`, `DateTimeConverter`, `FeatureFilter` (others fall back to schema-transparent passthrough; sources seed `open`). Tests: 271 (runtime+processor) + 6 (cli, incl. 2 PLATEAU e2e + 3 reference-validation e2e) + others, all green; fmt+clippy clean. Note the PLATEAU workflows themselves still report 0 errors because their sources are expression-driven (`open`); the error path is reachable wherever an `AttributeMapper` (or any future closed-schema producer) sits upstream of a reference. Remaining to widen coverage: seed closed schemas from typed sources (CSV header, DB catalog) and add more transfer functions.
