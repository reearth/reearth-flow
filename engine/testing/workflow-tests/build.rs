...
fn main() -> Result<()> {
    ...
    println!("cargo:rerun-if-changed=../data/testcases");
    ...
    // Crate is at testing/workflow-tests/, so parent() is testing/
    let testing_dir = Path::new(&manifest_dir).parent().unwrap();
    let testdata_dir = testing_dir.join("data/testcases");
    // Workflows are still in the legacy fixture location (2 levels up from testing/)
    let fixture_dir = testing_dir
        .parent()
        .unwrap()
        .join("runtime/examples/fixture");

    let test_cases = discover_tests(&testdata_dir)?;
    let generated_code = generate_test_code(&test_cases, &testdata_dir, &fixture_dir)?;
