...
pub(crate) async fn download_asset(
    ...
    let uris = asset
        ...
        .iter()
        .map(|f| {
            let path = Path::new(&asset.base_url).join(f);
            let path =
                path.to_str()
                    .ok_or(crate::errors::WorkerError::failed_to_donwload_asset_files(
                        "Failed to convert path",
                    ))?;
            let uri = Uri::from_str(path)
                .map_err(crate::errors::WorkerError::failed_to_donwload_asset_files)?;
            Ok((f.to_string(), uri))
        })
