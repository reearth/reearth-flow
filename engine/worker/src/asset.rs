...
use reearth_flow_worker::types::metadata::Asset;
...
pub(crate) async fn download_asset(
    ...
    let uris = asset
        ...
        .map(|f| {
            ...
            let path = path.to_str().ok_or(Error::failed_to_download_asset_files(
                "Failed to convert path",
            ))?;
            let uri = Uri::from_str(path).map_err(Error::failed_to_download_asset_files)?;
            Ok((f.to_string(), uri))
        })
    ...
    let futures = uris
        ...
        .map(|(name, uri)| async move {
            let storage = storage_resolver
                .resolve(uri)
                .map_err(Error::failed_to_download_asset_files)?;
            let bytes = storage
