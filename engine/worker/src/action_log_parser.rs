...
impl LogParser {
    ...
    pub fn parse(&self, log_line: &str) -> Option<LogPattern> {
        ...
        if let Some(caps) = self.node_terminate.captures(log_line) {
            let node_name = caps.get(2).unwrap().as_str().to_string();
            let elapsed =
                self.parse_duration(caps.get(4).unwrap().as_str(), caps.get(5).unwrap().as_str());
            return Some(LogPattern::NodeTerminate { node_name, elapsed });
        }
    ...
    fn parse_duration(&self, time_str: &str, unit: &str) -> Duration {
        ...
        // Clamp to reasonable bounds (0 to 1 day)
        let value = value.clamp(0.0, 86_400_000.0); // 1 day in ms
