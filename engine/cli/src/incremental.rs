...
pub fn rewrite_feature_store_file_paths_in_dir(
    ...
    let prev_job = previous_job_id.to_string();
    let cur_job = job_id.to_string();

    let prev_jobs_seg = format!("/jobs/{}/", prev_job);
    let cur_jobs_seg = format!("/jobs/{}/", cur_job);

    for entry in fs::read_dir(dir).map_err(crate::errors::Error::init)? {
...
fn rewrite_jsonl_file(path: &Path, prev_jobs_seg: &str, cur_jobs_seg: &str) -> crate::Result<()> {
    let file = fs::File::open(path).map_err(crate::errors::Error::init)?;
    let reader = BufReader::new(file);

    let tmp_path = tmp_jsonl_path(path);
    let tmp_file = fs::File::create(&tmp_path).map_err(crate::errors::Error::init)?;
    ...
    fs::rename(&tmp_path, path).map_err(crate::errors::Error::init)?;
    Ok(())
}
            ...
            for (k, val) in map.iter_mut() {
                if k == "filePath" {
                    if let serde_json::Value::String(s) = val {
                        let rewritten = rewrite_one_path(s, prev_jobs_seg, cur_jobs_seg);
                        *s = rewritten;
                    }
                } else {
...
fn rewrite_one_path(s: &str, prev_jobs_seg: &str, cur_jobs_seg: &str) -> String {
    ...
    if plain.contains("/artifacts/") {
        let alt_plain = plain.replace("/artifacts/", "/temp-artifacts/");
        if Path::new(&alt_plain).exists() {
            return format!("{prefix}{alt_plain}");
        }
    }

    if plain.contains("/temp-artifacts/") {
        let alt_plain = plain.replace("/temp-artifacts/", "/artifacts/");
        if Path::new(&alt_plain).exists() {
            return format!("{prefix}{alt_plain}");
        }
    }
