...
| I | Migration roadmap (confirmed 2026-06-17) | **Progressive by ENVIRONMENT, not per-cluster.** Sequence: (1) **Port** all flow-owned repos to Postgres while keeping the Mongo code intact (both backends, `DB_DRIVER`-selected; `mustComplete` gates `postgres` boot until all are ported). (2) **Replicate** current Mongo data into Postgres (per-entity backfill via the repo interfaces). (3) **Progressively deploy** `DB_DRIVER=postgres` per environment (dev → staging → prod) — each environment is single-backend, so a transaction never spans Mongo+Postgres (A1 global switch is correct; **no per-cluster/hybrid Transactor wiring needed**); Mongo kept hot for instant rollback. (4) **Remove** the Mongo support code once Postgres is stable across all envs. **`Asset` + `AssetUpload` are NOT ported** — externalized to reearth-serve (R2 + D1 asset platform) via a gateway, removed from `repo.Container` before cutover. Account + authserver remain reearthx's separate stream and gate the prod flip. |
