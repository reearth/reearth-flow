...
use crate::interface::http::types::{Document, HistoryItem};
...
impl DocumentHandler {
    ...
    pub async fn get_latest(
        ...
        if let Err(e) = state.document_service.flush_to_gcs(&doc_id).await {
            error!("Failed to flush websocket changes for '{}': {}", doc_id, e);
        }

        let result = async {
            match state.document_service.load_document(&doc_id).await? {
                Some(doc) => {
                    let read_txn = doc.transact();
                    let doc_state = read_txn.encode_diff_v1(&StateVector::default());
                    drop(read_txn);
                ...
                None => Err(anyhow::anyhow!("Document not found: {}", doc_id)),
            }
        }
        .await;
    ...
    pub async fn get_history(
        ...
        let result = async {
            let updates = state.document_service.get_updates(&doc_id).await?;
    ...
    pub async fn rollback(
        ...
        let rollback_result = async {
            let doc = state
                .document_service
                .rollback_document(&doc_id, version as u32)
                .await?;
    ...
    pub async fn get_history_metadata(
        ...
        let result = async {
            let metadata = state.document_service.get_updates_metadata(&doc_id).await?;
