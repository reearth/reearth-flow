...
impl<R: RedisRepository> RedisService<R> {
    ...
    pub async fn heartbeat(&self, doc_id: &str, instance_id: &str) -> Result<(), R::Error> {
        self.repository
    ...
    pub async fn delete_document_stream(&self, doc_id: &str) -> Result<(), R::Error> {
        self.repository.delete_stream(doc_id).await
    }
    ...
    pub async fn release_lock(&self, lock_key: &str, lock_value: &str) -> Result<(), R::Error> {
        self.repository.release_lock(lock_key, lock_value).await
    }
}

/// Convenience type alias for Redis service using the concrete RedisStore implementation.
pub type ConcreteRedisService = RedisService<crate::domain::entity::redis::RedisStore>;

impl ConcreteRedisService {
    /// Create a new Redis service from a RedisStore.
    pub fn from_store(store: crate::domain::entity::redis::RedisStore) -> Self {
